...
from clean_data import rm_ext_and_nan as rm, norm_standard
...
thresh = 15# FM can't be more than 15 per hour
...
X_test_norm = nsd(X_test, selected_feat=('LB', 'ASTV'), mode=mode, flag=False)
y_pred, w_norm_std = pred_log(logreg,X_train_norm,y_train,X_test_norm) # complete this function using nsd function
print("Accuracy is: " + str("{0:.2f}".format(100 * metrics.accuracy_score(y_test, y_pred))) + "%")
print("F1 score is: " + str("{0:.2f}".format(100 * metrics.f1_score(y_test, y_pred, average='macro'))) + "%")
